...
function vpl_prepare_update_events($vpl) {
    ...
    foreach ($overrides as $override) {
        ...
        vpl_update_events($vpl, $override);
    }
}
...
function vpl_update_events($vpl, $override = null) {
    ...
    $conds = array('modulename' => 'vpl', 'instance' => $vplinstance->id);
    if (!empty($override)) {
    ...
    if (!empty($vpl->get_course_module())) {
        $cmid = $vpl->get_course_module()->id;
    } else {
        $cmid = get_coursemodule_from_instance('vpl', $vplinstance->id, $vplinstance->course)->id;
    }

    foreach ($overrides as $current) {
        $groupid   = isset($current->groupid) ? $current->groupid : 0;
        $userid    = isset($current->userid) ? $current->userid : 0;
        $duedate = isset($current->duedate) ? $current->duedate : $vplinstance->duedate;
        ...
        $event->description = format_module_intro('vpl', $vplinstance, $cmid, false);
        $event->format = FORMAT_HTML;
        // Events module won't show user events when the courseid is nonzero.
        $event->courseid    = ($userid) ? 0 : $vplinstance->course;
        $event->groupid     = $groupid;
        $event->userid      = $userid;
        $event->modulename  = 'vpl';
        $event->instance    = $vplinstance->id;
        $event->timestart   = $duedate;
        $event->timeduration = 0;
        $event->timesort    = $event->timestart + $event->timeduration;
        $event->visible     = instance_is_visible('vpl', $vplinstance);
        $event->eventtype   = 'due';
        $event->priority    = null;
        ...
        if ($groupid) {
            ...
            $params = new stdClass();
            $params->assign = $vplinstance->name;
            $params->group = groups_get_group_name($groupid);
            if ($params->group === false) {
                // Group doesn't exist, just skip it.
                continue;
            }
            $eventname = get_string('overridegroupeventname', 'vpl', $params);
            ...
            $params = new stdClass();
            $params->assign = $vplinstance->name;
            $eventname = get_string('overrideusereventname', 'vpl', $params);
            // Set user override priority.
            $event->priority = CALENDAR_EVENT_USER_OVERRIDE_PRIORITY;
        } else {
            // The parent event.
            $eventname = $vplinstance->name;
        }
        ...
        if ($duedate && $addclose) {
            ...
            $event->name      = $eventname.' ('.get_string('duedate', 'vpl').')';
...
function vpl_extend_navigation(navigation_node $vplnode, $course, $module, $cm) {
    ...
    $node = vpl_navi_node_create($vplnode, 'useroverride', $url);
...
function vpl_extend_settings_navigation(settings_navigation $settings, navigation_node $vplnode) {
    ...
    if ($manager) {
        ...
        if (has_capability('mod/vpl:manageoverrides', $PAGE->cm->context)) {
            ...
            $groupoverride = vpl_navi_node_create($vplnode, 'groupoverrides', $url, navigation_node::TYPE_CONTAINER);
            $vplnode->add_node( $groupoverride, $fkn );

            // adding a navigation node for user override
            $useroverrideparams = array('cmid' => $cmid, 'mode' => 'user'); 
            $url = new moodle_url( '/mod/vpl/overrides.php', $useroverrideparams);
            $useroverride = vpl_navi_node_create($vplnode, 'useroverrides', $url, navigation_node::TYPE_CONTAINER);
            $vplnode->add_node( $useroverride, $fkn );
        }
